...
::::: {.myq}
**5.**     A random variable $X$ has bounded support, and a statistician wishes to estimate $\theta = \max X$, the maximum value $X$ can take. The statistician has access to 24 IID samples $X_1, X_2, \dots, X_{20}$ from this random variable which can be read into R as follows:
...
:::: {.subq}
**(a)**   Explain why the plug-in estimator is $\theta^* = \max \{X_j : j = 1, \dots, m\}$, and find the $\theta^*$ for this data.
::::
