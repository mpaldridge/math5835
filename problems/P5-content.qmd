...
::: myanswers
*Solution.* A Markov chain is a stochastic process $(X_n)$ satisfying the Markov property
$$ \mathbb P(X_{n+1} = x_{n+1} \mid X_n = x_n, \dots, X_1 = x_1) = \mathbb P(X_{n+1} = x_{n+1} \mid X_n = x_n) $$
for all states $x_{n+1}, x_n, \dots, x_1 \in \mathcal S$ such that the conditional probabilities are defined.
...
The process $(W_i)$ does not satisfy the Markov property. Suppose $Z_i = \pm 1$ with probability $\frac12$ each. Then $W_1 = Z_1$ and $W_2 = W_1 + Z_2 = Z_1 + Z_2$. Suppose $W_2 = 0$. Then the distribution of $W_3$ does not solely depend on $W_2$, conditionally independent of $W_1$. If $W_1 = Z_1 = +1$, then
$$ W_3 = \max\{W_1, W_2\} + Z_3 = \max\{1, 0\} + Z_3 = 1 + Z_3 , $$
while if $W_1 = Z_1 = -1$, then
$$ W_3 = \max\{W_1, W_2\} + Z_3 = \max\{-1, 0\} + Z_3 = Z_3 . $$
:::
::::
...
We form a Markov chain $(X_i)$ by making proposals that are accepted or rejected. From $X_i = x$ we propose a move to $y = x+1$ with probability $\frac12$ or to $y = x-1$ with probability $\frac12$. Such a proposal is accepted with probability
$$\alpha(x, y) = \min \left\{ \frac{\pi(y)}{\pi(x)} , \, 1 \right\} . $$
An acceptance means $X_{i+1} = y$, while a rejection means $X_{i+1} = x$. This is repeated $n$ times.
:::
::::
...
::: myanswers
*Solutions.* Proposals are made as in part (b) above. The acceptance probability for an "up" move is
\begin{align*}
...
\end{align*}
The acceptance probability for a "down" move is
\begin{align*}
\alpha(x, x-1) &= \min \left\{ \frac{\pi(x-1)}{\pi(x)} , \, 1 \right\} \\
&= \min \left\{ \frac{\mathrm{e}^{-\lambda} \lambda^{x-1}/(x-1)! }{\mathrm{e}^{-\lambda} \lambda^{x}/x!} , \, 1 \right\} \\
&= \min \left\{ \frac{x}{\lambda} , \, 1 \right\} .
\end{align*}
for $x > 0$, or $\alpha(0, -1) = 0$.
...
::: myanswers
*Solution.* We have
$$ \operatorname{Var} \left(\frac{1}{n} \sum_{i=1}^n \phi(X_i) \right) = \frac{1}{n^2} \left( \sum_{i=1}^n\operatorname{Var}\big(\phi(X_i)\big) + 2 \sum_{i < j} \operatorname{Cov}\big(\phi(X_i),\phi(X_j)\big) \right) . $$

Write $\operatorname{Var}(\phi(X)) = \sigma^2$. Suppose, seeking an approximation, that the Markov chain was started in equilibrium, so with $X_1$ distributed according to $\pi$. Since $\pi$ is a stationary distribution, we then have all $X_i$ distributed according to $\pi$. Then $\operatorname{Var}(\phi(X_i)) = \sigma^2$ for all $i$ and 
$$ \operatorname{Cov}\big(\phi(X_i),\phi(X_j)\big) = \rho(j-i) \, \sigma^2, $$ where $\rho(k) = \operatorname{Corr}(X_1, X_{k+1})$ is the autocorrelation at lag $k$. We then have
\begin{align*}
...
\end{align*}
We can approximate $\sum_{k=1}^{n-j} \rho(k)$ by $\sum_{k=1}^{\infty} \rho(k)$, since $n$ is large and the autocorrelation should decay quickly for a well-designed Markov chain. Hence
\begin{align*}
