...
:::: {.subq}
**(a)**  Defining any notation you use, write down the basic Monte Carlo estimator $\widehat\theta_n^{\mathrm{MS}}$ and the importance sampling estimator $\widehat\theta_n^{\mathrm{IS}}$ for an expectation of the form $\Exg \phi(X)$, where $X$ is a random variable with probability density function $f$.
