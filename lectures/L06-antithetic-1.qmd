...
(which makes $n/2 \times 2 = n$ samples overall, of course). Here, *within* each pair, $X_i$ and $X_i'$ are *not* independent, but *between* different pairs, $(X_i, X_i')$ and $(X_j, X'_j)$ *are* independent.
...
\begin{align*} \widehat{\theta}_n^{\mathrm{AV}} &= \frac{1}{n} \sum_{i=1}^{n/2} \big(\phi(X_i) + \phi(X'_i) \big) \\
...
* For antithetic variables Monte Carlo estimation, which take pairs of non-independent variables $X, X'$, to get the estimator
