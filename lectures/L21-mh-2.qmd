...
It's also worth doing short "pilot" runs, where you try different values of $\sigma$ and examine what seems to work best.
...
**Next time.** *We analyses the error of Monte Carlo estimation with the output of a Markov chain.*
...
- The Metropolis--Hastings algorithm in continuous space works essentially the same as in discrete space: propose a move according to the transition density $r(x,y)$, and accept it with probability $$\alpha(x,y) = \min \left\{ \frac{\pi(y)\,r(y,x)}{\pi(x)\,r(x,y)} , \, 1\right\} . $$
...
- Choosing the step size $\sigma$ in the Metropolis random walk is important: too small and the Markov chain takes only very small steps; too large and most proposals are rejected.
