...
The concept of a Markov chain and the Markov property remain the same: the next step $X_{i+1}$ may depend on the current step $X_i$, but, given the current step $X_i$, may not depend any further on on the past steps $X_{i-1}, X_{i-2}, \dots, X_2, X_1$. That is,
$$ \mathbb P(X_{i+1} \in A \mid X_i = x_i, X_{i-1} = x_{i=1}, \dots, X_1 = x_1) 
= \mathbb P(X_{i+1} \in A \mid X_i = x_i)$$ for all $i = 1, 2, \dots$ and for all $x_1, \dots, x_{i-1}, x_{i} \in \mathcal S$ and $A \subset \mathcal S$ such that the conditional probability is defined.
...
**Read more:** [Voss, *An Introduction to Statistical Computing*](https://leeds.primo.exlibrisgroup.com/permalink/44LEE_INST/1fj430b/cdi_askewsholts_vlebooks_9781118728031), Subsection 2.3.1.
:::
