...
::: {#def-MCest}
Let $X$ be a random variable, $\phi$ a function, and write $\theta = \Exg\phi(X)$. Suppose that $X_1, X_2, \dots, X_n$ are a random sample from $X$. Then the **Monte Carlo estimate** $\widehat\theta_n^{\mathrm{MC}}$ is
...
Our new Monte Carlo estimate is `r format(round(MCest, 5), nsmall = 5)`, which is closer to the true value of 2.
:::
