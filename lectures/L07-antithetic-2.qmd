...
3.  The mean-square error of $\widehat{\theta}_n^{\mathrm{MC}}$ is $$ \operatorname{MSE}\big(\widehat{\theta}_n^{\mathrm{AV}}\big) = \frac{1}{2n} \operatorname{Var}\big(\phi(X) + \phi(X')\big) = \frac{1+\rho}{n}\Var\big(\phi(X)\big). $$

4.  The root-mean-square error of $\widehat{\theta}_n^{\mathrm{MC}}$ is $$ \operatorname{RMSE}\big(\widehat{\theta}_n^{\mathrm{AV}}\big) = \frac{1}{\sqrt{2n}} \sqrt{\operatorname{Var}\big(\phi(X) + \phi(X')\big)} = \frac{\sqrt{1+\rho}}{\sqrt{n}}\sqrt{\Var\big(\phi(X)\big)}. $$
:::
...
* We saw that if $X \sim \operatorname{N}(0, 1)$ is a standard normal distribution, then $X' = -X \sim \operatorname{N}(0, 1)$ too. More generally, if $X\sim \operatorname{N}(\mu, \sigma^2)$, then $X' = 2\mu - X \sim \operatorname{N}(\mu, \sigma^2)$ can be used as an antithetic variable.

* We saw that if $U \sim \operatorname{U}[0, 1]$ is a continuous uniform distribution on $[0,1]$, then $U' = 1-U \sim \operatorname{U}[0, 1]$ too. More generally, if $X\sim \operatorname{U}[a, b]$, then $X' = (a + b) - X \sim \operatorname{U}[a, b]$ can be used as an antithetic variable.
...
A cunning trick is to introduce an IID copy of $U$; let's call it $V$. The first term in the above expression, $\Exg \phi(U)\phi(1-U)$, could also be written as $\Exg \phi(V)\phi(1-V)$; while the second term in the above expression, $\big(\Exg\phi(U)\big) \big(\Exg\phi(1-U)\big)$ could also be written as $\big(\Exg\phi(V)\big) \big(\Exg\phi(1-U)\big)$ or $\big(\Exg\phi(U)\big) \big(\Exg\phi(1-V)\big)$.

Taking advantage of these alternatives, we have
\begin{align*}
\operatorname{Cov}\big(\phi(U), \phi(U')\big)
&= \tfrac12 \big(\Exg \phi(U)\phi(1-U) + \Exg \phi(V)\phi(1-V) \big) \\
& \qquad {}- \tfrac12 \Big(\big(\Exg\phi(V)\big) \big(\Exg\phi(1-U)\big) - \big(\Exg\phi(U)\big) \big(\Exg\phi(1-V)\big) \Big) \\
&= \tfrac12 \Ex \big( \phi(U)\phi(1-U) + \phi(V)\phi(1-V) - \phi(V)\phi(1-U) \phi(U)\phi(1-V) \big) \\
&= \tfrac12 \Ex \big(\phi(U) - \phi(V)\big)\big(\phi(1-U) - \phi(1-V)\big) .
\end{align*}
In the first equality, we replaced each term by two of its alternatives, so multiplied by a half to take account for the doubled copies. In the second equality, we took advantage of the independence between $U$ and $V$ to take the expectation all the way outside. The third equality was simply factorising inside the expectation.

We now claim that the expectation in the final line above is negative. In fact, we have a stronger result: $$\big(\phi(U) - \phi(V)\big)\big(\phi(1-U) - \phi(1-V)\big)$${#eq-cross}
is *always* negative, so its expectation certainly is. To see this, think separately of the two cases $U \leq V$ and $U \geq V$.

* If $U \geq V$, then $\phi(U) \geq \phi(V)$ too, since $\phi$ is increasing. But, also this means that $1-V \geq 1-U$, so $\phi(1 - V) \geq \phi(1-U)$. This means that, in @eq-cross, the first term is positive and the second term is negative, so the product is negative.

* If $U \leq V$, then $\phi(U) \leq \phi(V)$ too, since $\phi$ is increasing. But, also this means that $1-V \leq 1-U$, so $\phi(1 - V) \leq \phi(1-U)$. This means that, in @eq-cross, the first term is negative and the second term is positive, so the product is negative.
...
* Let $U \sim \operatorname{U}[0, 1]$ and $\phi$ is monotonically increasing, the $\phi(U)$ and $\phi(1-U)$ are negatively correlated.

On Thursdays lecture, we will be discussing your answers to [**Problem Sheet 1**](#P1).
