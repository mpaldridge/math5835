...
(We did the "switching the dummy variable from $y$ to $x$" thing again.) So $Z = \Exg \phi(X)$. But that's no good: $\theta = \Exg \phi(X)$ was the thing we were trying to estimate in the first place. If we knew that, we wouldn't have to do Monte Carlo estimation to start with!
...
* If $X \sim \operatorname{Exp}(\lambda)$, then try $Y \sim \operatorname{N}(\mu)$ for some other value $\mu$.

## Bonus example

::: {#exm-ISnew}
*Let $X \sim \operatorname{U}[0, 10]$ be an uniform distribution, so $f(x) = \frac{1}{10}$ for $0 \leq x \leq 10$, and let $\phi(x) = \exp^{-|x-8|}Estimate $\Exg\phi(X).*
