...
Indeed, we have $$ F_X(x) = \mathbb P(X \leq x) = \mathbb P\big((b-a)U + a \leq x\big) = \mathbb P \left( U \leq \frac{x - a}{b-a}\right) . $$ But putting $u = (x - a)/(b - a)$ in @eq-cdf-u, does indeed give the CDF in @eq-cdf-x. Thw lower boundary $u < 0$ becomes $x < 0 \times (b-a) + a = a$; the upper boundary $u > b$ becomes $x > 1 \times(b-a) + a = b$; and, in between, the CDF $u$ becomes $(x - a)/(b - a)$. Thus we have proven that $X$ has the CDF of the $\operatorname{U}[a,b]$ distribution, as required.
...
::: {#def-cdf-inv}
Let $X$ be a random variable with cumulative distribution function $F_X(x) = \mathbb P(X \leq x)$. Then the **inverse cumulative distribution function** (inverse CDF) $F_X^{-1}$ is defined for $u \in (0,1)$ by $$ F^{-1}_X(u) = \inf \big \{x : F_X(x) \geq u \big\} . $$
:::
