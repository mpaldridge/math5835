...
Let's pick $X$ to be uniform on $[0,1]$. This means we should simply take $\phi(x) = h(x) = x^{1.6}(1-x)^{0.7}$, since $\frac{1}{b - a} = \frac{1}{1-0}$ is just 1 for us here. We can then approximate this integral in R using the Monte Carlo estimate
