...
While we've covered the most important facts about envelope rejection sampling last time, there are a few quick matters to mop up today.
...
$$ f(x) = \frac{1}{Z]\,\mu(x) , $$
where the **measure** $\mu$ is known, but the normalising constant
$$ Z = \int_{-\infty}^{+\infty}\mu(x) \,\mathrm{d}x $$
...
-   **We will need:** A random variable $Y$ with PDF $g$ that we can
    sample from, and a finite constant $c$ such that $\mu(x) \leq cg(x)$
    for all $x$.

-   **Step 1:** Sample a proposal $Y$ from the PDF $g$.

-   **Step 2:** Accept the proposal $Y$ with probability $\mu(Y) / cg(Y)$;
    otherwise reject $Y$. If accepted, *end*. If rejected, *go back to
    Step 1*.
:::
...
-->
::: {#exm-mises}
The **von Mises distribution** is a distribution on $[0, 2\pi)$ with PDF
...
* Conditions for an LCG to have full period of $m$ are given by the Hull--Dobell theorem: if $m$ is a power of 2, then we need $c$ to me even and $a$ to be $1 \bmod 4$.
