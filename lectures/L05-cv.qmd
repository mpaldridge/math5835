...
Let's recap where we've got to. The Monte Carlo estimate of $\theta = \Exg \phi(X)$ from IID samples $X_1, X_2, \dots, X_n$ is
...
::: {#def-MCest}
Let $X$ be a random variable, $\phi$ a function, and write $\theta = \Exg\phi(X)$. Let $\psi$ be a function fuch that $\Exg\psi(X)$ is known. Suppose that $X_1, X_2, \dots, X_n$ are a random sample from $X$. Then the **control variate Monte Carlo estimate** $\widehat\theta_n^{\mathrm{CV}}$ of $\theta$ is
