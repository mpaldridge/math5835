...
We still have a problem here, though. We (usually) don't know $\Var(\phi(X))$. But we can't even estimate $\phi(X)$ until we've already taken the samples. But we can use of this idea with a three-step process:
