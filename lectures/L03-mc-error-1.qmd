...
1. $\Ex \widehat{\theta}_n^{\mathrm{MC}} = \Ex \overline Y_m = \Ex Y = \Exg \phi(X)$, so $\operatorname{bias}(\widehat{\theta}_n^{\mathrm{MC}}) = \Exg \phi(X) - \Exg \phi(X) = 0$.
...
Then we can similar estimate the mean-square and root-mean-square errors by
$$ \frac{1}{n}S^2 \qquad \text{and} \qquad \sqrt{\frac{1}{n} S^2} = \frac{1}{\sqrt{n}}S  $$
respectively.
