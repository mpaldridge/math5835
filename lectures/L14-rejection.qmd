...
and that
$$ 0 \leq \sin^2x\leq 1 .$$
(Here, $\sin^2 x$ means $(\sin x)^2$, by the way.) This means that, if we take proposals $Y \sim \operatorname{N}(0,1)$, and then accept an proposed sample with probability $\alpha(x) = \sin^2 x$, that will give us the distribution @eq-distprop.
...
Let's examine these questions a bit closer. Write $a = \mathbb P(\text{accept})$ for the probability a sample $Y$ gets accepted (a priori, before we have seen the value $Y = x$ of that sample).  In the discrete case, this is
$$ a = \mathbb P(\text{accept}) = \sum_x \mathbb P(Y = x)\,\alpha(x) , $$
and in the discrete case this is
$$ a  = \mathbb P(\text{accept}) = \int_{-\infty}^{+\infty} g(x)\,\alpha(x)\,\mathrm{d}x . $$
In both cases, this can be written more succinctly as $a = \Exg\alpha(Y)$.
...
Unless $a$ is very close to $0$ or $1$, $N$ can be approximated by a normal distribution $N \approx \operatorname{N}(am, a(1-a)m)$. For large $m$, the standard deviation will be very small compared to the expectation, so the behaviour will be very predictable. (When $a$ is very small, $N$ can be better approximated by a Poisson distribution $N \approx \operatorname{Po}(am)$.)

The alternative way is the **"keep taking proposed samples until we have accepted enough of them"** way.

To get one sample requires a geometric $\operatorname{Geom}(a)$ number of samples. This is because the geometric distribution counts the number of trials needed until the first success. To get $n$ accepted samples will require the sum of $n$ independent $\operatorname{Geom}(a)$ distributions -- this is sometimes called the *negative binomial* distribution $M \sim \operatorname{NegBin}(n, a)$, which is the number of trials needed until the $n$th success. This has expectation $n/a$ and standard deviation $\sqrt{n(1-a)}/a$.

Unless $a$ is very close to $0$ or $1$, $M$ can be approximated can be approximated by a normal distribution $N \approx \operatorname{N}(am, a(1-a)m)$. For large $m$, the standard deviation will be very small compared to the expectation, so the behaviour will be very predictable. (When $a$ is very small, $M$ can be better approximated by a Gamma distribution.)
-->
::: {#exm-cond2}
...
From 1,000,000 samples, the number of accepted samples we might get is shown below.
...
plot(
  0:2e5, dbinom(0:2e5, m, acc_prob),
  type = "l", col = "blue",
  xlab = "number of accepted samples", ylab = "probability"
)
```

We see that the number of acceptance will be very close to $an = 0.158\times 1\,000\,000 = 158\,000$. The number of samples required for 100,000 acceptance is shown below.
...
acc_prob <- pnorm(1, lower.tail = FALSE)
plot(
  n + 0:7e5, dnbinom(0:7e5, n, acc_prob),
  type = "l", col = "blue", lwd = 2,
  xlab = "number of required samples", ylab = "probability"
)
```
...
::: {#exm-con3}
Suppose instead we were trying to sample from a standard normal conditional on $Y > 4$. Again, we take a standard normal and accept if the sample is greater than 4. This is a different matter, because the acceptance probability is very small: about $0.00003$.
...
plot(
  0:15, dbinom(0:15, m, acc_prob),
  type = "h", col = "blue", lwd = 2,
  xlab = "number of accepted samples", ylab = "probability"
)
```

There is likely to be a very small number of acceptance -- and around a 4% chance there are no acceptances at all. The distribution of the number of acceptances is roughly Poisson.

The number of samples required for just 5 acceptances, on the other hand, is:
...
acc_prob <- pnorm(4, lower.tail = FALSE)
plot(
  n + 0:6e5, dnbinom(0:6e5, n, acc_prob),
  type = "l", col = "blue", lwd = 2,
  xlab = "number of required samples", ylab = "probability"
)
```
...
This shows the danger of running rejection sampling when the probability of acceptance is very small.
:::

(Annoyingly, R defines the geometric distribution to be the number of failures *before* the first success -- 1 less then our definition -- and the negative binomial to be the number of failures *before* the $n$th success -- $n$ less than our definition. This is why `n + dnbinom()` was plotted for the two second graphs.)
...
**Read more:** [Voss, *An Introduction to Statistical Computing*](https://leeds.primo.exlibrisgroup.com/permalink/44LEE_INST/1fj430b/cdi_askewsholts_vlebooks_9781118728031), Section X.Y.
:::
