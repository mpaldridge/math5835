...
3.  Use these $B$ versions of the statistic to get a **bootstrap estimate** of the expectation, variance, or confidence interval. To estimate the expectation of the statistic $\mathbb ET = \mathbb ET(X_1, \dots, X_n)$, use the sample mean of the evaluated statistics $\overline{T^*} = \frac{1}{B} \sum_{k=1}^B T^*_B$. To estimate the variance $\operatorname{Var}(T)$ use the sample variance
$$ \sum{1}{B-1} \sum_{k=1}^B \big(T^*_k - \overline{T^*}\big) .$$
We'll come back to the confidence interval later.
