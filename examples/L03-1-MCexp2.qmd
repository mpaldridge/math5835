...
The first number is `var_est` $= `r signif(var_est, 4)`$, the sample variance of our $\phi(x_i)$s:\
...
The second number is `MSEest` $= `{r} signif(var_est/n, 4)`$, our estimate of the mean-square error. Since $\operatorname{MSE}(\widehat{\theta}_n^{\mathrm{MC}}) = \frac{1}{n} \operatorname{Var}(\phi(X))$, then $\frac{1}{n} S^2$ should be a good estimate of the MSE.
